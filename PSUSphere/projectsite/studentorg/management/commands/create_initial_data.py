...
class Command(BaseCommand):
    ...
    def create_students(self, count):
        ...
        for _ in range(count):
            ...
            student_id = f"{fake.random_int(2022,2024)}-{fake.random_int(1,8)}-{fake.random_number(digits=4)}",
            lastname = fake.last_name(),
            firstname = fake.first_name(),
            middlename = fake.last_name(),
            program = Program.objects.order_by('?').first()  # Corrected this line
        )
        self.stdout.write(self.style.SUCCESS(
    ...
    def create_membership(self, count):
        ...
        for _ in range(count):
            ...
            date_joined=fake.date_between(start_date="-2y", end_date="today")  # Fixed format
        )
        self.stdout.write(self.style.SUCCESS(
        'Initial data for organization members created successfully.'
    ))
